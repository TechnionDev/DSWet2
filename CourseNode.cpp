...
CourseNode::CourseNode(int numOfClasses, int course_id,ListNode* bottom_node) : course_id(course_id), numOfClasses(numOfClasses) {
...
void CourseNode::remove() {
    try {
        for (int i = 0; i < numOfClasses; i++) {
            course_Lectures[i]->get_location()->remove();
        }
    } catch (...) { throw; }
}
