...
bool operator==(const Key& key1, const Key& key2) {
    return key1.views == key2.views and key1.course_id == key2.course_id and
           key1.lecture_id == key2.lecture_id;
}
...
std::string to_string(const Key& key) {
    return std::to_string(key.views) + "_" + std::to_string(key.course_id) +
           "_" + std::to_string(key.lecture_id);
}
}  // namespace LecturesStats